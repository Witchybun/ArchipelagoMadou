...
def extend_fishsanity_locations(randomized_locations: List[LocationData], fishsanity: int, random: Random, mod_value: str):
    prefix = "Fishsanity: "
    if fishsanity == options.Fishsanity.option_none:
        return
    elif fishsanity == options.Fishsanity.option_legendaries:
        randomized_locations.extend(location_table[f"{prefix}{legendary.name}"] for legendary in legendary_fish if legendary.mod_name == mod_value)
    elif fishsanity == options.Fishsanity.option_special:
        randomized_locations.extend(location_table[f"{prefix}{special.name}"] for special in special_fish if special.mod_name == mod_value)
    elif fishsanity == options.Fishsanity.option_random_selection:
        randomized_locations.extend(location_table[f"{prefix}{fish.name}"]
                                    for fish in all_fish_items if random.random() < 0.4 and fish.mod_name == mod_value)
    elif fishsanity == options.Fishsanity.option_all:
        randomized_locations.extend(location_table[f"{prefix}{fish.name}"] for fish in all_fish_items if fish.mod_name == mod_value)
...
def extend_friendsanity_locations(randomized_locations: List[LocationData], friendsanity: int, mod_value: str):
...
def extend_friendsanity_pet_locations(randomized_locations: List[LocationData], friendsanity: int):
    exclude_non_bachelors = friendsanity == options.Friendsanity.option_bachelors
