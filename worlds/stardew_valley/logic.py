...
class StardewLogic:
    ...
    def __post_init__(self):
        ...
        self.quest_rules.update({
            ...
            "The Railroad Boulder": self.has("Furnace") & self.received("Skull Key"),
            "Grandpa's Shed": self.has_tool("Axe"),
            "Marlon's Boat": self.can_mine_in_the_mines_floor_81_120() & self.can_reach_region("Ginger Island") & self.can_reach_region("Adventurer's Guild"),
            "Aurora Vineyard": self.can_complete_community_center(),
            "Monster Crops": self.can_complete_quest("Marlon's Boat")
        })
    ...
    def can_catch_fish(self, fish: FishItem) -> StardewRule:
        ...
        if fish.locations == "Crimson Badlands":
            region_rule = region_rule & self.received("Skull Key") & self.has_lived_months(3) & self.has_galaxy_weapon()
        if fish.locations == "Highlands":
            region_rule = region_rule & self.can_earn_relationship("Lance", 2) | self.has_building("Highlands Obelisk")
        if fish.locations == "Fable Reef":
            region_rule = region_rule & self.can_earn_relationship("Lance", 8)
        if fish.locations == "Sprite Spring":
            region_rule = region_rule & self.can_earn_relationship("Wizard", 8)
    ...
    def can_meet(self, npc: str) -> StardewRule:
        ...
        if npc == "Apples":
            rules.append(self.can_complete_community_center() & self.has_lived_months(8))
        if npc == "Scarlett":
            rules.append(self.can_earn_relationship("Sophia",8))
        if npc == "Morgan":
            rules.append(self.has_lived_months(8))
        if npc == "Lance":
            rules.append(self.can_reach_region("Ginger Island"))
