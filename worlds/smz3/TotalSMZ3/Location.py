...
class Location:
    ...
    def AvailableWithinWorld(locations, items):
        result = []
        worldList = []
        [worldList.append(l.LunacidRegion.World) for l in locations if l.LunacidRegion.World not in worldList]
        for world in worldList:
            result += Location.AvailableGlobal([l for l in locations if l.LunacidRegion.World == world], [i for i in items if i.World == world])
        return result  
    ...
    def CanFillWithinWorld(locations, item, items):
        ...
        return [l for l in locations if l.CanFill(item, worldProgression[l.LunacidRegion.World.Id] and next(ll for ll in item.World.Locations if ll.Id == l.Id).Available(itemWorldProgression))]
